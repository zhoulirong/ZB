...
//主菜单--------------------------------------------------------------------------------------
	...
	var now_1=0;					//鼠标点击的序列号 (鼠标移开)
	var index_1=0;				   //鼠标移入的序列号
	var iClick=false;			
	

	$("ul.navigation li a").click(function(){		//主菜单点击  切换二级菜单的显示
		now_1=$(this).parent().index();
		$(this).addClass("on").parent().siblings().find("a").removeClass("on");	
		$(".sub_nav ul .sub_list p").eq(now_1).stop(true,true).show().siblings().hide();
		
		$(".sub_nav ul .sub_list p a").removeClass("on");
		$(".sub_nav ul .sub_list p").each(function(){
			$(this).find("a:first").addClass("on");		
		});
		$(".sub_nav ul .sub_list").find("p:first").find("a").removeClass("on"); 
		//document.title=now_1;
	});
	
	$("ul.navigation li a").mouseover(function(){	//鼠标移入
		index_1=$(this).parent().index();		
		$(".sub_nav ul .sub_list p").eq(index_1).stop(true,true).show().siblings().hide();
		if(index_1!=now_1){				
				$(".sub_nav ul .sub_list p").eq(now_1).siblings().find("a").removeClass("on");
		}
		iClick=false;
	});
			
	$("#headCont").hover(function(){	//这里一定要用hover，而不只是mouseout
	...
	$(".sub_nav ul .sub_list p a").click(function(){	//二级菜单点击 给对应的主菜单添加on样式
		iClick=true;
		$("ul.navigation li a").eq(index_1).addClass("on").parent().siblings().find("a").removeClass("on");	
	...
	//二级菜单，当滚动条滚到一定距离，让它相对浏览器定位(fixed)---------------------------------
	...
	function slide(slideMode){//轮播方法		
		if (isMoving==false){
			...
			if(slideMode){			//slideMode为1(true)，执行slide(1)，上一张
				...
				hidden.animate({width:"580px",height:"240px",top:"20px",left:"0px",opacity:1},timer);
				curr.animate({width:"580px",height:"240px",top:"20px",left:"600px",opacity:1},timer);
				next.animate({width:"450px",height:"180px",top:"60px","left":"730px","opacity":0},timer,function(){next.find("span").css("opacity",0); isMoving = false;});
				//prev  -->  curr     prev中的图片li轮换到curr的位置      其他一次轮换
				prev.find("span").css("opacity",0);
				$(".main_banner_box li").find("p").css({"bottom":"-50px"});//所有标题隐藏
				prev.animate({width:"670px",height:"280px",left:"255px",top:0,opacity:1},timer,function(){
				...
				hidden.animate({width:"580px",height:"240px",top:"20px",left:"600px",opacity:1},timer);							
				curr.animate({width:"580px",height:"240px",top:"20px",left:"0px",opacity:1},timer);
				//next  -->  curr     next中的图片li轮换到curr的位置      其他一次轮换
				next.find("span").css("opacity",0);
				$(".main_banner_box li").find("p").css({"bottom":"-50px"});//所有标题隐藏
				next.animate({width:"670px",height:"280px",left:"255px",top:0,opacity:1},timer,function(){
					$(this).find("p").animate({"bottom":"0px"});	//当前这张图片的标题运动出来
				});
				prev.animate({width:"450px",height:"180px",left:"0px",top:"60px",opacity:0},timer,function(){
					isMoving = false;
				}); 
				index_2++;	
			}//if else
	...
	//MV首播-----------------------------------------------------------------------------------
	var index_3=0;					//首播按钮序列号
	var index_33=0;				//热播按钮序列号（MV页面）
	var cont_1=$(".new_mv_body .mvList");
	var cont_2=$(".hot_mv_body .mvList");
	
	$(".new_mv_title2 ul li").click(function(){	//选项卡
		index_3=$(this).index();
